...
const ServiceItem = ({ service, isActive, onClick }) => {
  ...
  const iconStyle = isActive
    ...
    : "text-purple-400   transition duration-300";
  const arrowIcon = (
    <svg
  className={`w-9 h-9 transform transition-transform duration-700 ease-[cubic-bezier(0.45,0,0.55,1)] ${
  ...
  return (
    <div className="group">
      <div className={`cursor-pointer py-4`} onClick={() => onClick(service.id)}>
        <div className="flex justify-between items-center">
            <div className="flex items-center space-x-6">
                  <span className={`text-[22px] md:text-[28px] font-rale font-normal ${isActive ? "text-[#FFFFFF]" : "text-[#FFFFFF]"}`}>
                    {service.id.toString().padStart(2, "0")}
                  </span>
                  <h3 className="text-[22px] md:text-[28px] font-rale font-normal bg-clip-text text-transparent transition duration-300"
                    style={{
                      backgroundImage:
      ...
      {isActive && hasDetails && (
        <motion.div
          ...
          className="pl-16 pt-4 pb-8 space-y-6">
          {/* Descriptionnn............... */}
          <div className="border-l-4 border-purple-600 pl-6">
            <motion.p
                ...
                className="text-sm md:text-lg text-gray-400 max-w-3xl">
              {service.description}
            </motion.p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {service.subServices.slice(0, 3).map((sub, index) => (
              <div key={index} className="relative overflow-hidden rounded-lg group shadow-lg">
                {/* Image with texxttt.................. */}
                ...
                <div className="absolute inset-0  bg-opacity-60 flex items-center p-4 transition duration-300 group-hover:bg-opacity-40">
                  <motion.p
                   ...
                   className="text-[18px] font-medium font-rale  text-slate-200">{sub.name}</motion.p>
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <div className="text-white py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Services List */}
        <div className="space-y-0 font-radio">
          {" "}
